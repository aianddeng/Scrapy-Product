const getLink = ($, url) => {
    ...
    if (
        ...
        product_links = $('.srp-results .s-item__link').get().map(el => $(el).attr('href')).concat(
            $('.ebayui-pagination__ol a, .s-pagination .pagination__items a').get().map(el => $(el).attr('href'))
        );
    }
...
const handle = ($, url) => {
    ...
    const isProduct = $('#bc a [itemprop=name]').length + $('.breadcrumb li a').length;
