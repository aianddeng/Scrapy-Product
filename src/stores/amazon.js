const getLink = $ => {
    const product_links = $(
        '.s-search-results.s-result-list.s-main-slot .s-result-item[data-component-type=s-search-result] h2 a'
    )
        .get()
        .map(el => $(el).attr('href'))
        .concat(
        ...
        .map(el => 'https://www.amazon.com' + el)
...
const handle = ($, url) => {
    ...
    const isProduct =
        $('#productTitle').length + $('.a-breadcrumb .a-link-normal').length
