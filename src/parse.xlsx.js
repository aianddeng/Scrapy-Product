...
class Target {
    ...
    static loadList() {
        if (!Target.sheetDataMap) {
            ...
            let category_1 = null;
            let category_2 = null;
            const sheetDataMap = sheetData.slice().map(el => {
                if (el.length === 1) {
                    category_1 = el[0];
    ...
    static mkdir_category() {
        const baseUrl = path.join(__dirname, 'source');
    ...
    static async write_xlsx() {
        ...
        for (const categoryElem of categories) {
            ...
            if (tableList && tableList.length) {
                console.log(`(${++currentNum}/${categories.length}) ${categoryElem.catFull}: ${tableList.length}`);
                data[categoryElem.category] = tableList;
            } else {
                console.log(`(${++currentNum}/${categories.length}) ${categoryElem.catFull}: error`);
            }

            const paths = Target.loadList().find(
                el => el[2] === categoryElem.catFull
            );
            const filePath = path.join(__dirname, 'source', paths[0], paths[1], paths[2].replace(/(\/)/g, '-') + '.xlsx');
